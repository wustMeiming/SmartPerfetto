...
本次审查及独立复审共发现 18 类问题，均已在本次修改中修复。仓库新增
...
都有非空的 `zh-CN` 与 `en` 名称、描述和展示元数据；少量显式步骤描述也进入
相同门禁。
...
| I18N-03 | 内置 Skill 名称、步骤标题、列和聚合指标没有完整双语来源。 | 234 个内置 Skill | 新增从 registry 生成的严格本地化目录和 freshness 检查。 |
...
| I18N-17 | 首轮目录遗漏了 Skill `description`，英文列表探针中 234 个内置 Skill 有 224 个仍泄漏中文。 | Skill 列表、详情和意图识别 | 独立复审后补齐 Skill/步骤描述目录、运行时投影、全 registry 测试和门禁。 |
| I18N-18 | Skill REST 校验与失败响应仍硬编码英文。 | Skill 列表、详情、执行、分析、意图与厂商识别 | 统一按请求语言生成错误标题和参数说明，技术错误详情保持原样。 |
...
- display name、description、步骤标题/描述、显式列 label/tooltip 和聚合指标标签
  都具有两个语言版本。
...
精确命令结果由最终交付信息汇总；本文件记录可复现的门禁集合，不声称提交消息
包含验证日志。
